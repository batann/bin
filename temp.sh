...
echo -e "${Cyan}   >>>${White} 3${Cyan})${Blue} Add Tasknote to existing Tasks\033[0m"
echo -e "${Purple}+-----------------------------------------+${NC}"
read -p ' >>>     ' abc
if [[ $abc -eq 1 ]]; then
	while :; do
	...
	task ls
	tput cup 0 0
	echo -e "${White}\033[43mEnter [Task ID] to add Tasknote\033[0m"
	echo -e "${Blue}======================================${White}"
	echo -e ""
	read -p 'Enter [Task ID]    >>> ' cde
	if [[ -z $cde ]]; then
		sudo -u batan bash temp.sh
	fi
	tasknote $cde
